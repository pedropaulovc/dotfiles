...
run_success_case() {
  ...
  : >"$call_log"
  if ! MARKETPLACES="$marketplaces" CALL_LOG="$call_log" PATH="$fake_bin:$PATH" sh "$hook" >"$tmp_dir/$case_name-output" 2>&1; then
...
if MARKETPLACES="$wrong_source" CALL_LOG="$call_log" PATH="$fake_bin:$PATH" sh "$hook" >"$tmp_dir/wrong-output" 2>&1; then
  printf 'Different marketplace source unexpectedly succeeded.\n' >&2
  exit 1
fi
if ! awk '/Marketplace agent-plugins is registered with a different source\./ { found = 1 } END { exit !found }' "$tmp_dir/wrong-output"; then
  printf 'Different marketplace source produced the wrong error.\n' >&2
  cat "$tmp_dir/wrong-output" >&2
  exit 1
fi
if [ -s "$call_log" ]; then
...
printf '%s\n' 'POSIX OMP marketplace matching accepts URL and shorthand output, adds missing sources, and rejects mismatches.'
