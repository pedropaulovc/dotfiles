...
try {
    ...
    @(
        "omp-plugin-upgrade",
        "yof", "yoo", "yom", "yos", "yot", "yol", "yoa",
        "yoc", "yofc", "yooc", "yomc", "yosc", "yotc", "yolc", "yoac",
        "pyof", "pyoo", "pyom", "pyos", "pyot", "pyol", "pyoa",
        "pyoc", "pyofc", "pyooc", "pyomc", "pyosc", "pyotc", "pyolc", "pyoac",
        "yct", "yc-t", "ycft", "ycot", "ycst",
        "yx-t", "yxst", "yxtt", "yxlt", "yxat",
        "yo-t", "yoft", "yoot", "yomt", "yost", "yott", "yolt", "yoat",
        "pyo-t", "pyoft", "pyoot", "pyomt", "pyost", "pyott", "pyolt", "pyoat"
    ) | ForEach-Object {
...
'@
    ...
    yos --probe
    $calls = @(Get-Content -LiteralPath $ompCallLog)
    if ($calls.Count -ne 1 -or $calls[0] -ne "--auto-approve --model openai-codex/gpt-6-sol:medium --thinking medium --smol openai-codex/gpt-6-luna:max --slow openai-codex/gpt-6-sol:medium --plan openai-codex/gpt-6-astra:high --probe") {
        throw "yos did not select the medium-effort Codex role models."
    }
    Clear-Content -LiteralPath $ompCallLog
    yot --probe
    $calls = @(Get-Content -LiteralPath $ompCallLog)
    if ($calls.Count -ne 1 -or $calls[0] -ne "--auto-approve --model openai-codex/gpt-5.6-terra:medium --thinking medium --smol openai-codex/gpt-6-luna:max --slow openai-codex/gpt-6-sol:medium --plan openai-codex/gpt-6-astra:high --probe") {
